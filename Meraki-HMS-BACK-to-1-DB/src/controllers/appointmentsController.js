...
// Delete patient by phone or email
exports.deletePatient = async (req, res) => {
  try {
    const { phone, email, hospitalId } = req.body;
    ...
    if (!phone && !email) {
      return res
        .status(400)
        .json({ message: "Please provide phone or email to delete patient" });
    }

    // Find and delete patient by hospitalId + phone/email
    const deletedPatient = await ReceptionistPatient.findOneAndDelete({
      hospitalId,
      $or: [{ phone }, { email }],
    });
